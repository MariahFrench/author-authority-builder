...
import { T as __classPrivateFieldSet, a as promiseWithResolvers, h as AnthropicError, o as ToolError, u as loggerFor, w as __classPrivateFieldGet } from "./sdk-BzmOgV_-.js";
import { t as require_browser_external_node_fs } from "./browser-external_node_fs-CricAmDg.js";
import { t as require_browser_external_node_path } from "./browser-external_node_path-jhuYRk69.js";
//#region browser-external:node:fs/promises
var require_promises$1 = /* @__PURE__ */ __commonJSMin(((exports, module) => {
...
//#endregion
//#region browser-external:node:child_process
var require_browser_external_node_child_process = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = Object.create(new Proxy({}, { get(_, key) {
...
//#endregion
//#region browser-external:node:readline
var require_browser_external_node_readline = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = Object.create(new Proxy({}, { get(_, key) {
		if (key !== "__esModule" && key !== "__proto__" && key !== "constructor" && key !== "splice") console.warn(`Module "node:readline" has been externalized for browser compatibility. Cannot access "node:readline.${key}" in client code. See https://vite.dev/guide/troubleshooting.html#module-externalized-for-browser-compatibility for more details.`);
	} }));
}));
//#endregion
//#region node_modules/@anthropic-ai/sdk/helpers/beta/json-schema.mjs
var import_promises$1 = /* @__PURE__ */ __toESM(require_promises$1(), 1);
...
var import_browser_external_node_child_process = /* @__PURE__ */ __toESM(require_browser_external_node_child_process(), 1);
var import_browser_external_node_crypto = /* @__PURE__ */ __toESM(require_browser_external_node_crypto(), 1);
...
async function realpathOrSelf(p) {
	try {
		return await import_promises$1.realpath(p);
	} catch {
		return p;
	}
}
...
async function canonicalize(abs) {
	...
	for (;;) {
		let real;
		try {
			real = await import_promises$1.realpath(prefix);
		} catch {
			let isLink = false;
			try {
				isLink = (await import_promises$1.lstat(prefix)).isSymbolicLink();
			} catch {}
			if (isLink) {
				prefix = import_browser_external_node_path.resolve(import_browser_external_node_path.dirname(prefix), await import_promises$1.readlink(prefix));
				continue;
			}
			const parent = import_browser_external_node_path.dirname(prefix);
			if (parent === prefix) return abs;
			tail.push(import_browser_external_node_path.basename(prefix));
			prefix = parent;
			continue;
		}
		return tail.length ? import_browser_external_node_path.join(real, ...tail.reverse()) : real;
	}
}
/**
...
async function confineToRoot(root, p, opts) {
	...
	if (import_browser_external_node_path.isAbsolute(p)) {
		if (!allowOutside) throw new ToolError(`absolute path ${JSON.stringify(p)} not permitted`);
		return import_browser_external_node_path.resolve(p);
	}
	const realRoot = await realpathOrSelf(import_browser_external_node_path.resolve(root));
	const abs = import_browser_external_node_path.resolve(realRoot, p);
	if (allowOutside) return abs;
	const real = await canonicalize(abs);
	const rootSep = realRoot.endsWith(import_browser_external_node_path.sep) ? realRoot : realRoot + import_browser_external_node_path.sep;
	if (real !== realRoot && !real.startsWith(rootSep)) throw new ToolError(`path ${JSON.stringify(p)} escapes workdir`);
	return real;
}
/**
...
async function atomicWriteFile(targetPath, content) {
	const dir = import_browser_external_node_path.dirname(targetPath);
	const tempPath = import_browser_external_node_path.join(dir, `.tmp-${process.pid}-${(0, import_browser_external_node_crypto.randomUUID)()}`);
	let handle;
	try {
		handle = await import_promises$1.open(tempPath, "wx", 420);
		await handle.writeFile(content, "utf-8");
		await handle.sync();
		await handle.close();
		handle = void 0;
		await import_promises$1.rename(tempPath, targetPath);
	} catch (err) {
		if (handle) await handle.close().catch(() => {});
		await import_promises$1.unlink(tempPath).catch(() => {});
		throw err;
	}
}
/**
...
var require_browser_external_node_util = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = Object.create(new Proxy({}, { get(_, key) {
...
//#endregion
//#region browser-external:node:stream/promises
var require_promises = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	module.exports = Object.create(new Proxy({}, { get(_, key) {
		if (key !== "__esModule" && key !== "__proto__" && key !== "constructor" && key !== "splice") console.warn(`Module "node:stream/promises" has been externalized for browser compatibility. Cannot access "node:stream/promises.${key}" in client code. See https://vite.dev/guide/troubleshooting.html#module-externalized-for-browser-compatibility for more details.`);
	} }));
}));
//#endregion
...
var execFileAsync = (0, import_browser_external_node_util.promisify)(import_browser_external_node_child_process.execFile);
/**
...
async function setupSkills(ctx) {
	...
	const skillsRoot = import_browser_external_node_path.resolve(ctx.workdir, "skills");
	const created = [];
	for (const skill of session.agent.skills) try {
		...
		let dirname = import_browser_external_node_path.basename(version.name.trim());
		if (dirname === "" || dirname === "." || dirname === "..") dirname = skill.skill_id;
		const dest = import_browser_external_node_path.resolve(skillsRoot, dirname);
		if (dest !== skillsRoot && !dest.startsWith(skillsRoot + import_browser_external_node_path.sep)) {
		...
		await import_promises$1.rm(dest, {
			recursive: true,
			force: true
		});
		await import_promises$1.mkdir(dest, {
			recursive: true,
			mode: 493
		});
		created.push(dest);
	...
	return async () => {
		for (const dest of created) await import_promises$1.rm(dest, {
			recursive: true,
			force: true
		}).catch((e) => {
...
function assertSafeMemberNames(names) {
	for (const raw of names.split("\n")) {
		...
		if (!entry) continue;
		if (import_browser_external_node_path.isAbsolute(entry) || entry.split(/[\\/]/).includes("..")) throw new AnthropicError(`refusing to extract unsafe archive member: ${entry}`);
	}
}
/**
...
async function extractSkillArchive(resp, dest) {
	const tmp = import_browser_external_node_path.join(dest, `.skill-archive-${process.pid}-${Date.now()}`);
	if (!resp.body) throw new AnthropicError("skill download response had no body");
	await (0, import_promises.pipeline)(import_browser_external_node_stream.Readable.fromWeb(resp.body), import_browser_external_node_fs.createWriteStream(tmp));
	const stage = import_browser_external_node_path.join(import_browser_external_node_path.dirname(dest), `.skill-stage-${process.pid}-${Date.now()}`);
	try {
		...
		await import_promises$1.mkdir(stage, {
			recursive: true,
			mode: 493
		});
		...
		const srcRoot = top ? import_browser_external_node_path.join(stage, top) : stage;
		for (const entry of await import_promises$1.readdir(srcRoot)) await import_promises$1.rename(import_browser_external_node_path.join(srcRoot, entry), import_browser_external_node_path.join(dest, entry));
	} finally {
		await import_promises$1.rm(tmp, { force: true });
		await import_promises$1.rm(stage, {
			recursive: true,
			force: true
		});
	}
...
async function readHead(file, n) {
	const handle = await import_promises$1.open(file, "r");
	try {
...
var BashSession = class {
	constructor(dir, env = scrubbedShellEnv()) {
		...
		__classPrivateFieldSet(this, _BashSession_proc, import_browser_external_node_child_process.spawn("/bin/bash", ["--noprofile", "--norc"], {
			cwd: dir,
			env: {
				...env,
				PS1: "",
	...
	async exec(command, opts = {}) {
		...
		const sentinel = `__ANT_CMD_${import_browser_external_node_crypto.randomUUID()}_DONE__`;
...
function betaReadTool(ctx) {
	return betaTool({
		...
		run: async ({ file_path, view_range }) => {
			...
			let data;
			try {
				const st = await import_promises$1.stat(abs);
				if (!st.isFile()) throw new ToolError(`read: ${file_path} is not a regular file`);
				if (st.size > READ_MAX_BYTES) throw new ToolError(`read: ${file_path} is ${st.size} bytes, exceeds ${READ_MAX_BYTES}-byte limit. Use bash (head/tail/sed) to read a slice.`);
				data = await import_promises$1.readFile(abs, "utf8");
			} catch (e) {
				if (e instanceof ToolError) throw e;
...
function betaWriteTool(ctx) {
	return betaTool({
		...
		run: async ({ file_path, content }) => {
			...
			const abs = await resolvePath(ctx, file_path);
			try {
				await import_promises$1.mkdir(import_browser_external_node_path.dirname(abs), {
					recursive: true,
					mode: 493
				});
	...
	return betaTool({
		...
		run: async ({ file_path, old_string, new_string, replace_all }) => {
			...
			try {
				...
				if (st.size > EDIT_MAX_BYTES) throw new ToolError(`edit: ${file_path} is ${st.size} bytes, exceeds ${EDIT_MAX_BYTES}-byte limit. Use bash (sed/awk) to edit a large file.`);
				data = await import_promises$1.readFile(abs, "utf8");
			} catch (e) {
				if (e instanceof ToolError) throw e;
...
function betaGlobTool(ctx) {
	return betaTool({
		...
		run: async ({ pattern, path: searchPath }) => {
			...
			let root = import_browser_external_node_path.resolve(ctx.workdir);
			let pat = pattern;
			if (import_browser_external_node_path.isAbsolute(pattern)) {
				if (!ctx.unrestrictedPaths) throw new ToolError("glob: absolute pattern not permitted");
				root = import_browser_external_node_path.parse(pattern).root;
				pat = import_browser_external_node_path.relative(root, pattern);
			...
			try {
				for await (const entry of fsGlob(pat, {
					...
					const full = import_browser_external_node_path.join(entry.parentPath, entry.name);
					if (!ctx.unrestrictedPaths && !isWithin(root, full)) continue;
					let mtime = 0;
					try {
						mtime = (await import_promises$1.stat(full)).mtimeMs;
					} catch {}
					matches.push({
						path: full,
						mtime
...
function betaGrepTool(ctx) {
	return betaTool({
		...
		run: async ({ pattern, path: p }, context) => {
			...
			let searchPath = import_browser_external_node_path.resolve(ctx.workdir);
...
function runRipgrep(rg, pattern, searchPath, signal) {
	return new Promise((resolve, reject) => {
		const proc = import_browser_external_node_child_process.spawn(rg, [
			"-n",
			"--no-heading",
			"-e",
			pattern,
...
async function runWalkGrep(pattern, root, signal) {
	...
	if ((await import_promises$1.stat(root).catch(() => null))?.isFile()) await grepFile(root, re, push);
	else await walk(root, "", (rel) => grepFile(import_browser_external_node_path.join(root, rel), re, push), signal);
	if (signal?.aborted) throw new ToolError("grep: aborted");
	if (hits.length === 0) return "no matches";
	return hits.join("\n");
}
async function grepFile(file, re, push) {
	const stream = import_browser_external_node_fs.createReadStream(file, { encoding: "utf8" });
	const rl = import_browser_external_node_readline.createInterface({
		input: stream,
		crlfDelay: Infinity
	});
	let i = 0;
...
function isWithin(root, p) {
	const rel = import_browser_external_node_path.relative(root, p);
	return rel === "" || !rel.startsWith(".." + import_browser_external_node_path.sep) && rel !== ".." && !import_browser_external_node_path.isAbsolute(rel);
}
...
async function walk(root, rel, fn, signal) {
	...
	async function inner(rel, depth) {
		...
		let entries;
		try {
			entries = await import_promises$1.readdir(import_browser_external_node_path.join(root, rel), { withFileTypes: true });
		} catch {
			return true;
		}
		for (const e of entries) {
			...
			if (signal?.aborted) return false;
			const childRel = rel ? import_browser_external_node_path.join(rel, e.name) : e.name;
			if (e.isDirectory()) {
...
async function findRg() {
	const dirs = (process.env["PATH"] ?? "").split(import_browser_external_node_path.delimiter);
	for (const d of dirs) {
		const candidate = import_browser_external_node_path.join(d, "rg");
		try {
			await import_promises$1.access(candidate, import_browser_external_node_fs.constants.X_OK);
			return candidate;
		} catch {}
	}
	return null;
...
//#endregion
export { betaAgentToolset20260401, setupSkills };

//# sourceMappingURL=node-Dd-jZyrK.js.map